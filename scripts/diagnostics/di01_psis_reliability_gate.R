...
phase_begin("di01", "PSIS reliability gate")
ensure_analysis_dirs()

script_name <- "scripts/diagnostics/di01_psis_reliability_gate.R"
...
if (nrow(loo_comp) > 0) {
  empirical_rows <- bind_rows(lapply(seq_len(nrow(loo_comp)), function(i) {
    ...
    data.frame(
      source_script = "scripts/ma09_loo_stacking.R",
...
if (nrow(sim) > 0) {
  make_sim_rows <- function(kind) {
    ...
    bind_rows(lapply(seq_len(nrow(sim)), function(i) {
      ...
      data.frame(
        source_script = "scripts/simulation/si03_brms_leakage_confirmation.R",
...
phase_end("di01", "PSIS reliability gate")
