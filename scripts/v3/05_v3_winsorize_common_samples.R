# -----------------------------------------------------------------------------
# Script: 05_v3_winsorize_common_samples.R
# Purpose: Winsorize v3 common samples at 1st/99th percentiles before any
...
sample_specs <- data.frame(
  ...
  Input = c(
    "final_v3_common_ex_post_sample.csv",
    "final_v3_common_realtime_sample.csv",
    "final_v3_secondary_operating_cycle_ex_post_sample.csv",
    "final_v3_secondary_operating_cycle_realtime_sample.csv",
    "final_v3_M08_ex_post_subsample.csv",
    "final_v3_M08_realtime_subsample.csv"
  ),
  Output = c(
    "final_v3_common_ex_post_sample_winsor.csv",
    "final_v3_common_realtime_sample_winsor.csv",
    "final_v3_secondary_operating_cycle_ex_post_sample_winsor.csv",
    "final_v3_secondary_operating_cycle_realtime_sample_winsor.csv",
    "final_v3_M08_ex_post_subsample_winsor.csv",
    "final_v3_M08_realtime_subsample_winsor.csv"
  ),
  Sample_Group = c(
    "main_common",
    "main_common",
...
for (i in seq_len(nrow(sample_specs))) {
  spec <- sample_specs[i, ]
  input_path <- file.path(v3_original_root, "tables", spec$Input)
  output_path <- file.path(v3_winsor_root, "tables", spec$Output)
...
write.csv(desc_df, file.path(v3_winsor_root, "tables", "table_v3_winsor_before_after_descriptives.csv"), row.names = FALSE)
write.csv(cutoff_df, file.path(v3_winsor_root, "tables", "table_v3_winsor_cutoffs.csv"), row.names = FALSE)
write.csv(sd_df, file.path(v3_winsor_root, "tables", "table_v3_winsor_sd_shrinkage.csv"), row.names = FALSE)

appendix_df <- desc_df[desc_df$Variable %in% appendix1_vars_v3, ]
appendix_df <- appendix_df[order(appendix_df$Sample, match(appendix_df$Variable, appendix1_vars_v3)), ]
write.csv(appendix_df, file.path(v3_winsor_root, "tables", "table_v3_winsor_appendix1_descriptives_corrected.csv"), row.names = FALSE)

sample_summary <- do.call(rbind, lapply(seq_len(nrow(sample_specs)), function(i) {
  spec <- sample_specs[i, ]
  path <- file.path(v3_winsor_root, "tables", spec$Output)
...
write.csv(sample_summary, file.path(v3_winsor_root, "tables", "table_v3_winsor_sample_summary.csv"), row.names = FALSE)

tail_audit_rows <- list()
for (i in which(sample_specs$Sample_Group == "secondary_operating_cycle")) {
  spec <- sample_specs[i, ]
  before <- read.csv(file.path(v3_original_root, "tables", spec$Input), stringsAsFactors = FALSE)
  after <- read.csv(file.path(v3_winsor_root, "tables", spec$Output), stringsAsFactors = FALSE)
...
write.csv(do.call(rbind, tail_audit_rows), file.path(v3_winsor_root, "tables", "table_v3_operating_cycle_tail_audit_winsor.csv"), row.names = FALSE)

formulas_path <- file.path(v3_original_root, "tables", "table_v3_named_model_formulas.csv")
registry_path <- file.path(v3_original_root, "tables", "table_v3_model_registry.csv")
...
write.csv(formulas_df, file.path(v3_winsor_root, "tables", "table_v3_named_model_formulas_winsor.csv"), row.names = FALSE)
...
for (col in colnames(registry_df)) {
  if (is.character(registry_df[[col]])) {
    ...
    registry_df[[col]] <- gsub("Breuer-Schutt \\(2023\\)", "Breuer & Schütt (2021/2022)", registry_df[[col]])
    registry_df[[col]] <- gsub("Real-time No-lead \\(v3\\)", "No-look-ahead feature set (v3 implementation)", registry_df[[col]])
  }
}
write.csv(registry_df, file.path(v3_winsor_root, "tables", "table_v3_model_registry_winsor.csv"), row.names = FALSE)
...
notes <- c(
  ...
  "Registry:",
  "  Registry copied to v3_winsor with requested label corrections.",
  "",
  paste("Outputs written under", v3_winsor_root, "only.")
)
writeLines(notes, con = file.path(v3_winsor_root, "logs", "v3_phase1b_winsor_notes.txt"))

cat("\n[SUCCESS] Phase 1b winsorized samples and audit tables created under ", v3_winsor_root, "/.\n", sep = "")
