...
write_pipeline_index <- function() {
  ...
  pipeline <- data.frame(
    Order = c(sprintf("%02d", 0:11), "13", "28", "31", "32", "21", "30", "C3", "12", sprintf("%02d", 14:20), sprintf("%02d", 22:27), "29"),
    Script = c(
      "00_helpers.R",
      ...
      "31_construct_exact_kfold_DA.R",
      "32_audit_DA_finite_outputs.R",
  ...
  readme_lines <- c(
    ...
    "",
    "`scripts/ma15_audit_DA_finite_outputs.R` writes `table_DA_finite_gate_decision.csv` and is a hard RQ2/export gate. Script `30` is a hard new-firm tail-suppression gate; if unverified Firm-RE out-of-firm posterior predictive tail quantities require suppression, export stops unless the explicit suppression override is set and the outputs are labelled non-primary. `Rscript run.R all --dry-run` de-duplicates script `30_new_firm_predictive_integration_audit.R` so the new-firm audit appears once.",
    "",
    "LOFO (`scripts/robustness/ro01_lofo_stacking.R`) is an opt-in robustness branch, not a default main step. Sensitivity scripts 14-20 and simulation scripts 23-27 are opt-in branches. PSIS reliability (`scripts/diagnostics/di01_psis_reliability_gate.R`) is secondary diagnostics, not the primary RQ1 comparison.",
    "",
