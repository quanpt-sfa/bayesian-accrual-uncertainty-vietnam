...
rows <- lapply(seq_len(nrow(tasks)), function(i) {
  ...
  data.frame(
    ...
    Main_Stack_Inclusion = if ("Main_Stack_Inclusion" %in% names(row)) row$Main_Stack_Inclusion else TRUE,
    Secondary_Robustness = if ("Secondary_Robustness" %in% names(row)) row$Secondary_Robustness else FALSE,
    ...
    original_elpd = if ("elpd_loo" %in% names(row)) row$elpd_loo else NA_real_,
    original_k_above_07 = if ("pareto_k_above_07" %in% names(row)) row$pareto_k_above_07 else NA_integer_,
    diagnostics_status = if ("diagnostics_status" %in% names(row)) row$diagnostics_status else NA_character_,
