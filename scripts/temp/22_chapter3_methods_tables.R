...
prediction_audit <- bind_rows(
  audit_prediction("scripts/12_lofo_stacking.R", "LOFO"),
  audit_prediction("scripts/13_grouped_kfold_firm.R", "grouped_kfold")
)
...
preprocessing_audit <- bind_rows(
  data.frame(
    ...
    script = "scripts/05_winsorize_common_samples.R",
  ...
  data.frame(
    ...
    script = "scripts/13_grouped_kfold_firm.R",
    ...
    required_action = ifelse(!is.null(kfold_std_audit), "None for grouped K-fold standardization audit.", "Run scripts/13_grouped_kfold_firm.R to produce train-standardization audit."),
    stringsAsFactors = FALSE
  )
)
...
report_lines <- c(
  ...
  "",
  "These tables were generated by `source(\"scripts/temp/22_chapter3_methods_tables.R\")` from existing raw data, common-sample, winsorization, prior predictive, LOFO, grouped K-fold, and manifest outputs. The script does not refit Bayesian models or alter empirical design choices.",
  "",
  "## Generated Files",
