...
main <- function() {
  ...
  lofo_ep <- read.csv(input_paths[["lofo_ex_post"]], stringsAsFactors = FALSE)
  lofo_rt <- read.csv(input_paths[["lofo_no_lookahead"]], stringsAsFactors = FALSE)
  ...
  make_fold_assignment <- function(df_ep, df_rt) {
    ...
    firm_summary <- combined %>%
      ...
      summarise(
        ...
        Dominant_Industry = if ("industry" %in% names(cur_data())) {
  ...
  fold_balance_one <- function(df, target_space) {
    df %>%
      ...
      summarise(
        ...
        Industry_Firm = if ("industry" %in% names(cur_data())) names(sort(table(industry), decreasing = TRUE))[1] else NA_character_,
        .groups = "drop"
      ) %>%
      group_by(Fold_ID) %>%
      summarise(
  ...
  build_task_manifest <- function(tasks) {
    out <- tasks %>%
      ...
      select(Target_Space, Sample_Group, Fold_ID, Model_ID, Model_Name, Heterogeneity_Variant,
             ...
             Divergences, Treedepth_Warnings, brms_Formula, Target_Sample)
    out
  }
  ...
  if (file.exists(manifest_path) && force_resume) {
    ...
    task_manifest <- task_manifest %>%
      left_join(old %>% select(any_of(c(join_cols, "Status", "Started_At", "Ended_At", "Runtime_Seconds", "N_Train_Obs",
                                        ...
                                        "Max_Rhat", "Divergences", "Treedepth_Warnings"))) %>%
