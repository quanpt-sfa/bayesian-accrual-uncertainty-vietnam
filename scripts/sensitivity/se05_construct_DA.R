...
phase_begin("se05", "Sensitivity construct DA")
ensure_analysis_dirs()
ensure_sensitivity_dirs()
validate_final_analysis_config("se05 sensitivity DA", final_mode = TRUE)
...
if (!file.exists(weights_path)) stop("[BLOCKER] Missing se04 sensitivity stacking weights. Run script 17 first.")
...
compute_stacked_da <- function(df_sample, weights_space, scenario, space_name) {
  ...
  if (nrow(active) == 0) stop("[BLOCKER] No finite active se04 sensitivity stacking weights for ", scenario, " / ", space_name)
...
phase_end("se05", "Sensitivity construct DA")
