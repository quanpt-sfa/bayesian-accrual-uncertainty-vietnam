...
phase_begin("se04", "Sensitivity stacking")
ensure_analysis_dirs()
ensure_sensitivity_dirs()
validate_final_analysis_config("se04 sensitivity stacking", final_mode = TRUE)
...
if (!file.exists(diag_path)) stop("[BLOCKER] Missing se03 sensitivity diagnostics. Run script 16 first.")
...
if (!dry_run && (!requireNamespace("brms", quietly = TRUE) || !requireNamespace("loo", quietly = TRUE))) {
  stop("[BLOCKER] brms and loo are required for non-dry-run se04 sensitivity stacking.")
}
...
resolve_sensitivity_kfold_run_root <- function(scenario) {
  ...
  if (identical(base_root_norm, baseline_root)) {
    stop("[BLOCKER] grouped_kfold for se04 sensitivity stacking cannot reuse baseline K-fold outputs. Generate scenario-specific exact K-fold outputs under '",
         file.path(sensitivity_root(scenario), "kfold_firm"),
...
phase_end("se04", "Sensitivity stacking")
