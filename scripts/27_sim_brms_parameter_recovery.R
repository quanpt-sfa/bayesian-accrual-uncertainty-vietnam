...
# Purpose: Bayesian MCMC parameter-recovery simulation for the Firm-RE Student-t
...
simulate_accrual_panel_student_truth <- function(n_firms = 80, T = 7, sigma_firm = 0.10,
  ...
  # data-generating residual below uses sigma_eps as scale, not as unconditional SD.
...
extract_recovery <- function(fit, truth, T, sigma_firm, rep_id, n_firms, n_obs, chains, iter, warmup) {
  draws <- posterior::as_draws_df(fit)
  rows <- lapply(seq_len(nrow(truth)), function(i) {
    v <- truth$draw_var[i]
    if (!v %in% names(draws)) {
      return(data.frame(parameter = truth$parameter[i], draw_var = v, true_value = truth$true_value[i],
                        ...
                        missing_draw = TRUE, boundary_parameter = truth$boundary_parameter[i]))
    }
    ...
    data.frame(
      ...
      coverage_95 = (tv >= unname(qs[1]) && tv <= unname(qs[3])),
      missing_draw = FALSE, boundary_parameter = truth$boundary_parameter[i]
    )
  })
  out <- dplyr::bind_rows(rows)
  out$T <- T; out$sigma_firm <- sigma_firm; out$rep_id <- rep_id
  out$n_firms <- n_firms; out$n_obs <- n_obs; out$chains <- chains; out$iter <- iter; out$warmup <- warmup
  out$error <- NA_character_
  out[, c("T", "sigma_firm", "rep_id", "n_firms", "n_obs", "chains", "iter", "warmup",
          "parameter", "draw_var", "true_value", "posterior_mean", "posterior_sd", "q025", "q500", "q975",
          "bias", "abs_bias", "squared_error", "coverage_95", "missing_draw", "boundary_parameter", "error")]
}

extract_diagnostics <- function(fit, T, sigma_firm, rep_id, n_firms, n_obs, chains, iter, warmup) {
  draws <- posterior::as_draws_df(fit)
  ...
  data.frame(
    ...
    max_treedepth_hit = sum(np$Parameter == "treedepth__" & np$Value >= max(np$Value[np$Parameter == "treedepth__"], na.rm = TRUE), na.rm = TRUE),
    error = NA_character_, stringsAsFactors = FALSE
  )
}

run_one_recovery_replication <- function(T, sigma_firm, rep_id, n_firms, n_industries, sigma_eps, nu,
  ...
  truth <- attr(df, "truth")
  list(
    recovery = extract_recovery(fit, truth, T, sigma_firm, rep_id, n_firms, nrow(df), chains, iter, warmup),
    diagnostics = extract_diagnostics(fit, T, sigma_firm, rep_id, n_firms, nrow(df), chains, iter, warmup)
  )
}
...
summarise_recovery <- function(x) {
  ...
  dplyr::as_tibble(ok) |>
    ...
    dplyr::summarise(
      ...
      coverage_95 = mean(.data$coverage_95, na.rm = TRUE),
...
message("Bayesian parameter recovery: ", nrow(grid), " MCMC fits. This can be slow.")
rec_out <- list(); diag_out <- list()
for (i in seq_len(nrow(grid))) {
  ...
  ans <- tryCatch(
    ...
    error = function(e) {
      ...
      list(
        recovery = data.frame(T = g$T, sigma_firm = g$sigma_firm, rep_id = g$rep_id,
        ...
        diagnostics = data.frame(T = g$T, sigma_firm = g$sigma_firm, rep_id = g$rep_id,
                                 n_firms = n_firms, n_obs = n_firms * g$T, chains = chains, iter = iter, warmup = warmup,
                                 max_rhat = NA_real_, min_ess_bulk = NA_real_, min_ess_tail = NA_real_,
                                 n_divergent = NA_real_, max_treedepth_hit = NA_real_, error = err)
      )
    }
  )
...
manifest <- data.frame(
  script = "scripts/27_sim_brms_parameter_recovery.R",
  ...
  replications = R, n_firms = n_firms, n_industries = n_industries,
  sigma_eps = sigma_eps, nu = nu, chains = chains, iter = iter, warmup = warmup,
  output_root = root,
