...
6. Execution configuration is centralized in `scripts/00_helpers.R`: use `accrual_base_seed()`, `accrual_seed()`, `accrual_seed_for()`, `set_accrual_seed()`, `accrual_sampler_config()`, `accrual_kfold_config()`, and `main_model_ids_for_space()` as the source of truth. The pipeline uses one canonical seed, `ACCRUAL_SEED`, with default `42`, and helper-owned seed setting is the only accepted RNG entrypoint.
