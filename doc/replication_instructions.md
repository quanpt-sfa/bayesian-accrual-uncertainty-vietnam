...
8. Script `31` writes exact-KFold DA source, draw-hash, IO, and model-inclusion manifests under `out/interim/winsor/tables`. `table_model_primary_inclusion_gate.csv` controls whether MCMC `REVIEW`/`CAUTION` models are retained with `MCMC_REVIEW_INCLUDED_WITH_EXACT_REFIT_PASS` or excluded.
9. Final baseline DA outputs are written to `accruals/baseline`; exact-KFold DA provenance outputs are written under `out/interim/winsor/tables`.
