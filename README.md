...
Execution configuration is centralized in `scripts/00_helpers.R`. The pipeline uses one canonical seed, `ACCRUAL_SEED`, with default `42`; `accrual_base_seed()` and `accrual_seed()` return that same seed for baseline, grouped exact K-fold, row exact K-fold, sensitivity, and simulation branches. Branch-specific seed env vars such as `ACCRUAL_BASELINE_SEED`, `ACCRUAL_KFOLD_FIRM_SEED`, `ACCRUAL_ROW_KFOLD_SEED`, `ACCRUAL_SENS_SEED`, and `ACCRUAL_SIM_SEED` are deprecated and blocked if they differ from `ACCRUAL_SEED`, to avoid branch-specific tuning or cherry-picking concerns. Sampler settings are read through `accrual_sampler_config()`, exact K-fold K/seed/sampler settings through `accrual_kfold_config()`, and primary model sets through `main_model_ids_for_space()`. The helper registry writes `out/manifests/method_design/execution_config_registry.csv`.
